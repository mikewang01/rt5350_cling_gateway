        ...
        if(ret == 0) {
            ...
            if(((ret) < 0)) {
				if( (errno == EINPROGRESS || errno == EALREADY)){
					...
					/*connectin timeout happens*/
					SET_NETWORK_STATE(socket_inf, STATE_INT_FIALD);
				}else if(errno == ECONNREFUSED){
					/*this means connection refuse*/
					SET_NETWORK_STATE(socket_inf, STATE_INT_FIALD);				
				}
        ...
        if(ret > 0) {
            ...
            if(FD_ISSET(socket_inf.sock_fd, &r_set)) {
                if ((n = read(socket_inf.sock_fd, (void*)p_ibuffer, lenth)) < 0) {
					if (n == 0){
						log_printf("connection has been closed\n");
					}else if(n == EAGAIN){
						log_printf("data never get reahced;internet has been disconnected; server has ben crashed\n");
						
					}else if(n == ECONNRESET){
						log_printf("server has been crashed\n");
					}else if(n == EINVAL){
						log_printf("server has closed connection\n");
					}					
   					RESET_NEWORK_PARA(socket_inf);
                } else {
                    p_ibuffer[n] = 0;
...
#define THREAD_SOCKET_TIMEOUT_VALUE 10000
    ...
    pipe_wset = pipe_rset;
    struct timeval socket_pipe_time;
        ...
        if(GET_NETWORK_STATE(socket_inf) == STATE_ETH_DOWN || GET_NETWORK_STATE(socket_inf) == STATE_INT_FIALD) {
            ...
            if(ret < 0) {
                ...
                if(GET_NETWORK_STATE(socket_inf) == STATE_ETH_DOWN) {
                	...
                	log_printf("net work glitch happened\n");
